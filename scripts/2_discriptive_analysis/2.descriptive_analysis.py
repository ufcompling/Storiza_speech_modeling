...
for i in range(len(userID_list)):
	userID_grade_dict[userID_list[i]] = grade_list[i]
...
children_produced_story_list = [] ## children whose data we ended up using

story_error_categories_dict = {}
story_error_labels_dict = {}
for audio in unique_original_audios:
	userID = audio.split('_')[1]
	if userID in userID_grade_dict:
		children_produced_story_list.append(userID)
	...
	for i in range(len(word_path_list)):
		...
		if path.startswith(audio_name):
			...
			story_error_categories_dict[audio].append(error_categories)
		...
		#            story_error_categories_dict[audio].append(category)
			try:
				...
				for label in error_labels:
					story_error_labels_dict[audio].append(label)
			except Exception:
				pass
		else:
			pass
...
for k, v in userID_grade_dict.items():
	if k in children_produced_story_list:
		if v == 'Kindergarten':
...
for k, v in story_error_categories_dict.items():
	if v and story_duration_dict.get(k):
		for userID, grade in userID_grade_dict.items():
			if k.split('_')[1] == userID and grade == '3rd Grade and Higher': 
...
# (7) Word correct per minute
if word_correct_per_min_list:
	word_correct_per_min_list.sort()
	print(f"(7) Average WCPM: {round(statistics.mean(word_correct_per_min_list),2)}")
	print('Lowest word-correct-per-minute (WCPM):', round(word_correct_per_min_list[0]))
	print('Highest word-correct-per-minute (WCPM):', round(word_correct_per_min_list[-1]))

else:
	print("(7) Average WCPM: N/A (no data)")
