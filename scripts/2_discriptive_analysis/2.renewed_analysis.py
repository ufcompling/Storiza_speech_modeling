...
PATTERN = re.compile(r"^uid_([^_]+)_sid_([^_]+)_([^_]+)_([^_]+)_sentence_segment\.wav$")

WKEY = re.compile(r"^uid_([^_]+)_sid_([^_]+)_([^_]+)$")

def parse_filename(filename: str):
    m = PATTERN.match(str(filename).strip())
    if not m:
        return None, None
    uid = m.group(1)
    audio_id = m.group(3)
...
if __name__ == "__main__":
    ...
    in_path = Path(os.path.join(PROCESSED_DIR, "utterance_windows.tsv"))
